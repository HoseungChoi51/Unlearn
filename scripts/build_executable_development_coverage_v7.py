#!/usr/bin/env python3
"""Build or check the immutable v7 coverage and v6-to-v7 migration locks."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--coverage-output",
        type=Path,
        default=ROOT / COVERAGE_V7_CONFIG_RELATIVE_PATH,
    )
    parser.add_argument(
        "--migration-output",
        type=Path,
    ...
    coverage = executable_development_coverage_v7_config_bytes()
    migration = (
        executable_development_coverage_v6_to_v7_migration_config_bytes()
    )
    if arguments.check:
        ...
        atomic_publish_noreplace(
            arguments.migration_output,
            migration,
            ".cbds-coverage-v6-to-v7-migration-",
        )
    return 0
