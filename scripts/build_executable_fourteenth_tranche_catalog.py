#!/usr/bin/env python3
"""Build or check the bounded hash-only fourteenth-tranche catalog report.

Catalog construction and validation live in
``cbds.executable_fixture_fourteenth_catalog``.  This script serializes
only that module's public hash projection and delegates no-replace
publication to the shared hardened helper.  It never writes fixture bytes,
oracle answers, prompts, or execution authority, and it never executes a
candidate.
"""
...
FOURTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES = 1024 * 1024
FOURTEENTH_TRANCHE_TEMPORARY_PREFIX = ".cbds-fourteenth-"
FourteenthTrancheCatalogPublicationError = HashOnlyReportPublicationError
...
def canonical_fourteenth_tranche_catalog_bytes() -> bytes:
    ...
    if len(payload) > FOURTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES:
        raise FourteenthTrancheCatalogPublicationError(
            "fourteenth-tranche report exceeds its fixed byte bound"
        )
    return payload
...
def read_existing_report(path: Path) -> bytes | None:
    ...
    return _shared_read_existing_regular(
        path,
        FOURTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES,
    )
...
def atomic_publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if (
        ...
        and len(payload) > FOURTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES
    ):
        raise FourteenthTrancheCatalogPublicationError(
            "fourteenth-tranche report exceeds its fixed byte bound"
        )
    _shared_atomic_publish_noreplace(
        path,
        payload,
        FOURTEENTH_TRANCHE_TEMPORARY_PREFIX,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = canonical_fourteenth_tranche_catalog_bytes()
    try:
        existing = read_existing_report(output)
    except FourteenthTrancheCatalogPublicationError as exc:
        raise SystemExit(str(exc)) from exc
    if existing is not None:
        if existing != payload:
            raise SystemExit(
                "existing fourteenth-tranche report differs from fresh generation"
            )
        return 0
    if arguments.check:
        raise SystemExit("fourteenth-tranche catalog report does not exist")
    try:
