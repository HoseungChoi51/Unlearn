#!/usr/bin/env python3
"""Build or check the immutable v3 coverage and v2-to-v3 migration locks."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--coverage-output",
        type=Path,
        default=ROOT / COVERAGE_V3_CONFIG_RELATIVE_PATH,
    )
    parser.add_argument(
        "--migration-output",
        type=Path,
    ...
    coverage = executable_development_coverage_v3_config_bytes()
    migration = (
        executable_development_coverage_v2_to_v3_migration_config_bytes()
    )
    if arguments.check:
        ...
        atomic_publish_noreplace(
            arguments.migration_output,
            migration,
            ".cbds-coverage-v2-to-v3-migration-",
        )
    return 0
