#!/usr/bin/env python3
"""Build or check the immutable v2 coverage and v1-to-v2 migration locks."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--coverage-output",
        type=Path,
        default=ROOT / COVERAGE_V2_CONFIG_RELATIVE_PATH,
    )
    parser.add_argument(
        "--migration-output",
        type=Path,
        default=ROOT / COVERAGE_MIGRATION_CONFIG_RELATIVE_PATH,
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    coverage = executable_development_coverage_v2_config_bytes()
    migration = executable_development_coverage_migration_config_bytes()
    if arguments.check:
        _check_exact(
