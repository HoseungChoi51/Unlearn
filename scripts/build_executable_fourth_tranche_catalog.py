#!/usr/bin/env python3
"""Build or check the hash-only additive fourth-tranche catalog report.

Catalog construction and validation live exclusively in
``cbds.executable_fixture_fourth_catalog``.  This script serializes only that
...
class FourthTrancheCatalogPublicationError(ValueError):
    """Raised when a report cannot be safely published without replacement."""


def canonical_fourth_tranche_catalog_bytes() -> bytes:
    """Return the central catalog's deterministic hash-only JSON projection."""

    catalog = build_fourth_tranche_fixture_catalog()
    return (
        json.dumps(
...
def _write_all(descriptor: int, payload: bytes) -> None:
    ...
    while view:
        ...
        if written <= 0:
            raise OSError("short write while publishing fourth-tranche report")
        view = view[written:]
...
def atomic_publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if not isinstance(path, Path) or type(payload) is not bytes:
        raise FourthTrancheCatalogPublicationError(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = canonical_fourth_tranche_catalog_bytes()
    if output.exists():
        if output.read_bytes() != payload:
            raise SystemExit(
                "existing fourth-tranche report differs from fresh generation"
            )
        return 0
    if arguments.check:
        raise SystemExit("fourth-tranche catalog report does not exist")
    try:
        atomic_publish_noreplace(output, payload)
    except FourthTrancheCatalogPublicationError as exc:
