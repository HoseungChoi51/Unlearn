#!/usr/bin/env python3
"""Build or check the immutable v4 coverage and v3-to-v4 migration locks."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--coverage-output",
        type=Path,
        default=ROOT / COVERAGE_V4_CONFIG_RELATIVE_PATH,
    )
    parser.add_argument(
        "--migration-output",
        type=Path,
    ...
    coverage = executable_development_coverage_v4_config_bytes()
    migration = (
        executable_development_coverage_v3_to_v4_migration_config_bytes()
    )
    if arguments.check:
        ...
        atomic_publish_noreplace(
            arguments.migration_output,
            migration,
            ".cbds-coverage-v3-to-v4-migration-",
        )
    return 0
