#!/usr/bin/env python3
"""Build or check the immutable v6 coverage and v5-to-v6 migration locks."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--coverage-output",
        type=Path,
        default=ROOT / COVERAGE_V6_CONFIG_RELATIVE_PATH,
    )
    parser.add_argument(
        "--migration-output",
        type=Path,
    ...
    coverage = executable_development_coverage_v6_config_bytes()
    migration = (
        executable_development_coverage_v5_to_v6_migration_config_bytes()
    )
    if arguments.check:
        ...
        atomic_publish_noreplace(
            arguments.migration_output,
            migration,
            ".cbds-coverage-v5-to-v6-migration-",
        )
    return 0
