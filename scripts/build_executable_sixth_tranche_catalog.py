#!/usr/bin/env python3
"""Build or check the hash-only additive sixth-tranche catalog report.

Catalog construction and validation live exclusively in
``cbds.executable_fixture_sixth_catalog``.  This script serializes only that
...
class SixthTrancheCatalogPublicationError(ValueError):
    """Raised when a report cannot be safely published without replacement."""


def canonical_sixth_tranche_catalog_bytes() -> bytes:
...
def _write_all(descriptor: int, payload: bytes) -> None:
    ...
    while view:
        ...
        if written <= 0:
            raise OSError("short write while publishing sixth-tranche report")
        view = view[written:]
...
def atomic_publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if not isinstance(path, Path) or type(payload) is not bytes:
        raise SixthTrancheCatalogPublicationError(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = canonical_sixth_tranche_catalog_bytes()
    if output.exists():
        if output.read_bytes() != payload:
            raise SystemExit(
                "existing sixth-tranche report differs from fresh generation"
            )
        return 0
    if arguments.check:
        raise SystemExit("sixth-tranche catalog report does not exist")
    try:
        atomic_publish_noreplace(output, payload)
    except SixthTrancheCatalogPublicationError as exc:
