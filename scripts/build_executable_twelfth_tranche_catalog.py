#!/usr/bin/env python3
"""Build or check the bounded hash-only twelfth-tranche catalog report.

Catalog construction and validation live in
``cbds.executable_fixture_twelfth_catalog``.  This script serializes only
...
TWELFTH_TRANCHE_REPORT_MAXIMUM_BYTES = 1024 * 1024
TWELFTH_TRANCHE_TEMPORARY_PREFIX = ".cbds-twelfth-"
TwelfthTrancheCatalogPublicationError = HashOnlyReportPublicationError
...
def canonical_twelfth_tranche_catalog_bytes() -> bytes:
    ...
    if len(payload) > TWELFTH_TRANCHE_REPORT_MAXIMUM_BYTES:
        raise TwelfthTrancheCatalogPublicationError(
            "twelfth-tranche report exceeds its fixed byte bound"
        )
    return payload
...
def read_existing_report(path: Path) -> bytes | None:
    ...
    return _shared_read_existing_regular(
        path,
        TWELFTH_TRANCHE_REPORT_MAXIMUM_BYTES,
    )
...
def atomic_publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if (
        ...
        and len(payload) > TWELFTH_TRANCHE_REPORT_MAXIMUM_BYTES
    ):
        raise TwelfthTrancheCatalogPublicationError(
            "twelfth-tranche report exceeds its fixed byte bound"
        )
    _shared_atomic_publish_noreplace(
        path,
        payload,
        TWELFTH_TRANCHE_TEMPORARY_PREFIX,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = canonical_twelfth_tranche_catalog_bytes()
    try:
        existing = read_existing_report(output)
    except TwelfthTrancheCatalogPublicationError as exc:
        raise SystemExit(str(exc)) from exc
    if existing is not None:
        if existing != payload:
            raise SystemExit(
                "existing twelfth-tranche report differs from fresh generation"
            )
        return 0
    if arguments.check:
        raise SystemExit("twelfth-tranche catalog report does not exist")
    try:
