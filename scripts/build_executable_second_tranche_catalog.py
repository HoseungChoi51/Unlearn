#!/usr/bin/env python3
"""Build or check a hash-only additive second-tranche catalog report.

Catalog construction and validation live exclusively in
``cbds.executable_fixture_second_catalog``.  This script only serializes that
module's hash-only projection and atomically publishes it at a caller-selected
path; fixture inputs and trusted oracle bytes are never written.
"""
...
class SecondTrancheCatalogPublicationError(ValueError):
    """Raised when a report cannot be safely published without replacement."""


def canonical_second_tranche_catalog_bytes() -> bytes:
...
def _write_all(descriptor: int, payload: bytes) -> None:
    ...
    while view:
        ...
        if written <= 0:
            raise OSError("short write while publishing second-tranche report")
        view = view[written:]
...
def atomic_publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if not isinstance(path, Path) or type(payload) is not bytes:
        raise SecondTrancheCatalogPublicationError(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = canonical_second_tranche_catalog_bytes()
    if output.exists():
        if output.read_bytes() != payload:
            raise SystemExit(
                "existing second-tranche report differs from fresh generation"
            )
        return 0
    if arguments.check:
        raise SystemExit("second-tranche catalog report does not exist")
    try:
        atomic_publish_noreplace(output, payload)
    except SecondTrancheCatalogPublicationError as exc:
