#!/usr/bin/env python3
"""Build or check a hash-only additive third-tranche catalog report.

Catalog construction and validation live exclusively in
``cbds.executable_fixture_third_catalog``.  This script only serializes that
...
class ThirdTrancheCatalogPublicationError(ValueError):
    """Raised when a report cannot be safely published without replacement."""


def canonical_third_tranche_catalog_bytes() -> bytes:
...
def _write_all(descriptor: int, payload: bytes) -> None:
    ...
    while view:
        ...
        if written <= 0:
            raise OSError("short write while publishing third-tranche report")
        view = view[written:]
...
def atomic_publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if not isinstance(path, Path) or type(payload) is not bytes:
        raise ThirdTrancheCatalogPublicationError(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = canonical_third_tranche_catalog_bytes()
    if output.exists():
        if output.read_bytes() != payload:
            raise SystemExit(
                "existing third-tranche report differs from fresh generation"
            )
        return 0
    if arguments.check:
        raise SystemExit("third-tranche catalog report does not exist")
    try:
        atomic_publish_noreplace(output, payload)
    except ThirdTrancheCatalogPublicationError as exc:
