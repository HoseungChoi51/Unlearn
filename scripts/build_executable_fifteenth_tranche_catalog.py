#!/usr/bin/env python3
"""Build or check the bounded hash-only fifteenth-tranche catalog report.

Catalog construction and validation live in
``cbds.executable_fixture_fifteenth_catalog``.  This script serializes
...
FIFTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES = 1024 * 1024
FIFTEENTH_TRANCHE_TEMPORARY_PREFIX = ".cbds-fifteenth-"
FifteenthTrancheCatalogPublicationError = HashOnlyReportPublicationError
...
def canonical_fifteenth_tranche_catalog_bytes() -> bytes:
    ...
    if len(payload) > FIFTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES:
        raise FifteenthTrancheCatalogPublicationError(
            "fifteenth-tranche report exceeds its fixed byte bound"
        )
    return payload
...
def read_existing_report(path: Path) -> bytes | None:
    ...
    return _shared_read_existing_regular(
        path,
        FIFTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES,
    )
...
def atomic_publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if (
        ...
        and len(payload) > FIFTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES
    ):
        raise FifteenthTrancheCatalogPublicationError(
            "fifteenth-tranche report exceeds its fixed byte bound"
        )
    _shared_atomic_publish_noreplace(
        path,
        payload,
        FIFTEENTH_TRANCHE_TEMPORARY_PREFIX,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = canonical_fifteenth_tranche_catalog_bytes()
    try:
        existing = read_existing_report(output)
    except FifteenthTrancheCatalogPublicationError as exc:
        raise SystemExit(str(exc)) from exc
    if existing is not None:
        if existing != payload:
            raise SystemExit(
                "existing fifteenth-tranche report differs from fresh generation"
            )
        return 0
    if arguments.check:
        raise SystemExit("fifteenth-tranche catalog report does not exist")
    try:
