...
| Generator-backed benchmark | Tests semantic programs and edge cases rather than prompt-template similarity | 300 integrated public-development tasks/1,500 fixtures across six additive tranches; the full 25-family/500-task allocation is locked | Gated: 10 families/200 tasks remain planned, independent human review is unfinished, and no sealed suite exists | Implement the locked remainder, review the complete inventory, then build closed ID/OOD suites |
...
| Oracle and semantic verifier | Decides whether output and filesystem state satisfy the task rather than merely resemble a reference string | Independent constructions and full-catalog mutation checks exist for fifteen integrated families | Gated: family coverage and stratified human review are incomplete | Finish semantic coverage, mutation audit, and external human review before sealing |
...
review, sealing, execution, scoring, selection, or claim status to the 10
planned families. The next locked family is `case-routed-batch-transform`.
