"""Auditable migration evidence from development coverage v7 to v8.

The migration changes exactly one of 25 family values.  It proves that the
planned process-lifecycle-delta contract is preserved during promotion,
...
from . import executable_development_coverage_v8 as coverage_v8
...
COVERAGE_V7_TO_V8_MIGRATION_SCHEMA_VERSION: Final[str] = "1.0.0"
COVERAGE_V7_TO_V8_MIGRATION_VERSION: Final[str] = "1.0.0"
COVERAGE_V7_TO_V8_MIGRATION_CONFIG_RELATIVE_PATH: Final[str] = (
    "configs/executable-method-development-coverage-v7-to-v8-migration.json"
)
MAXIMUM_COVERAGE_V7_TO_V8_MIGRATION_CONFIG_BYTES: Final[int] = 64 * 1024

FROZEN_V7_PROCESS_LIFECYCLE_FAMILY_SHA256: Final[str] = (
    "b7f590855836e878258829209e707f3b1b4c00bdc22d6352c65ce3e7f4f88c46"
)
FROZEN_V8_PROCESS_LIFECYCLE_FAMILY_SHA256: Final[str] = (
    "f5ef27ca10caedbad1e6e3652e6020c12524309698970438946969cf89310510"
)
FROZEN_COVERAGE_V7_TO_V8_MIGRATION_SHA256: Final[str] = (
...
MIGRATION_REASON_CODES: Final[tuple[str, ...]] = (
    ...
    "process-lifecycle-delta-discrimination",
    "fifteenth-tranche-integration",
...
class ExecutableDevelopmentCoverageV7ToV8MigrationError(ValueError):
    """Raised when the v7-to-v8 migration evidence is not exact."""
...
class ExecutableDevelopmentCoverageV7ToV8Migration:
    ...
    preserved_promotion_history: tuple[
        coverage_v8.CoveragePromotionEvidence, ...
    ]
    promotion_evidence: coverage_v8.CoveragePromotionEvidence
    ...
    schema_version: str = COVERAGE_V7_TO_V8_MIGRATION_SCHEMA_VERSION
    migration_version: str = COVERAGE_V7_TO_V8_MIGRATION_VERSION
    ...
    def __post_init__(self) -> None:
        validate_executable_development_coverage_v7_to_v8_migration(self)
...
def _migration_record(
    migration: ExecutableDevelopmentCoverageV7ToV8Migration,
) -> dict[str, object]:
    return {
        "schema_version": migration.schema_version,
        "migration_version": migration.migration_version,
        "record_type": (
            "cbds.executable-method-development-coverage-v7-to-v8-migration"
        ),
        "from": {
            ...
            "config_bytes_sha256": (
                coverage_v8.PREDECESSOR_CONFIG_BYTES_SHA256
            ),
            "config_byte_count": coverage_v8.PREDECESSOR_CONFIG_BYTE_COUNT,
            "git_commit": coverage_v8.PREDECESSOR_GIT_COMMIT,
        },
        "to": {
            "coverage_version": coverage_v8.COVERAGE_V8_VERSION,
            "coverage_sha256": migration.to_coverage_sha256,
            "config_relative_path": (
                coverage_v8.COVERAGE_V8_CONFIG_RELATIVE_PATH
            ),
        },
        "changed_family_count": 1,
        "unchanged_family_count": 24,
        "changed_family_id": "process-lifecycle-delta",
...
def compute_executable_development_coverage_v7_to_v8_migration_sha256(
    record: object,
) -> str:
    if type(record) is not dict:
        raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
...
def _migration_component_snapshot_bytes() -> bytes:
    ...
    old_coverage = coverage_v7.build_executable_development_coverage_v7()
    new_coverage = coverage_v8.build_executable_development_coverage_v8()
    if (
        old_coverage.coverage_sha256
        != coverage_v8.PREDECESSOR_COVERAGE_SHA256
        or new_coverage.coverage_sha256
        != coverage_v8.FROZEN_COVERAGE_V8_SHA256
        ...
        or len(new_coverage.promotion_history) != 6
        or [
            item.to_record()
        ...
        raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
            "live v7/v8 chain is not an exact append-only migration"
        )
    index = coverage_v8.PROCESS_LIFECYCLE_FAMILY_INDEX
    ...
    if (
        ...
        != coverage_v8.FROZEN_PROCESS_LIFECYCLE_TASK_SET_SHA256
        or _preserved_contract(old_family)
        != _preserved_contract(new_family)
    ):
        raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
            "process-lifecycle family promotion did not preserve its contract"
        )
    unchanged: list[str] = []
    for family_index, (old, new) in enumerate(
        ...
        if family_index == index:
            if old == new:
                raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
                    "changed process-lifecycle family is unexpectedly "
                    "identical"
                )
            continue
        if old != new:
...
def _migration_components() -> tuple[
    CoverageFamily,
    CoverageFamily,
    tuple[coverage_v8.CoveragePromotionEvidence, ...],
    coverage_v8.CoveragePromotionEvidence,
    SourceRegistryCommitment,
    tuple[str, ...],
]:
    try:
        ...
        if type(history_value) is not list or len(history_value) != 5:
            raise TypeError("preserved history must contain five entries")
        history = tuple(
            coverage_v8._promotion_from_record(item)
            for item in history_value
        )
        promotion = coverage_v8._promotion_from_record(
...
def validate_executable_development_coverage_v7_to_v8_migration(
    migration: ExecutableDevelopmentCoverageV7ToV8Migration,
) -> None:
    if type(migration) is not ExecutableDevelopmentCoverageV7ToV8Migration:
        raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
    ...
    if (
        ...
        != COVERAGE_V7_TO_V8_MIGRATION_SCHEMA_VERSION
        or type(migration.migration_version) is not str
        or migration.migration_version
        != COVERAGE_V7_TO_V8_MIGRATION_VERSION
    ...
    if (
        ...
        or any(
            type(item) is not coverage_v8.CoveragePromotionEvidence
            for item in migration.preserved_promotion_history
        )
        or type(migration.promotion_evidence)
        is not coverage_v8.CoveragePromotionEvidence
        ...
        raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
            "migration identities differ from live v7/v8 evidence"
        )
    for evidence in migration.preserved_promotion_history:
    ...
    if (
        ...
        != FROZEN_COVERAGE_V7_TO_V8_MIGRATION_SHA256
        or migration.migration_sha256
        != compute_executable_development_coverage_v7_to_v8_migration_sha256(
            record
        )
    ):
        raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
...
def build_executable_development_coverage_v7_to_v8_migration(
) -> ExecutableDevelopmentCoverageV7ToV8Migration:
    old, new, history, promotion, source, unchanged = (
        _migration_components()
    )
    provisional = ExecutableDevelopmentCoverageV7ToV8Migration.__new__(
        ExecutableDevelopmentCoverageV7ToV8Migration
    )
    values: dict[str, object] = {
        "from_coverage_sha256": coverage_v8.PREDECESSOR_COVERAGE_SHA256,
        "to_coverage_sha256": coverage_v8.FROZEN_COVERAGE_V8_SHA256,
        ...
        "schema_version": COVERAGE_V7_TO_V8_MIGRATION_SCHEMA_VERSION,
        "migration_version": COVERAGE_V7_TO_V8_MIGRATION_VERSION,
    ...
    return ExecutableDevelopmentCoverageV7ToV8Migration(
        from_coverage_sha256=coverage_v8.PREDECESSOR_COVERAGE_SHA256,
        to_coverage_sha256=coverage_v8.FROZEN_COVERAGE_V8_SHA256,
        old_family_sha256=old.family_sha256,
        new_family_sha256=new.family_sha256,
...
def load_executable_development_coverage_v7_to_v8_migration(
    ...
    try:
        payload = coverage_v8._read_stable_regular(
            source,
            MAXIMUM_COVERAGE_V7_TO_V8_MIGRATION_CONFIG_BYTES,
        )
    except coverage_v8.ExecutableDevelopmentCoverageV8Error as exc:
        raise ExecutableDevelopmentCoverageV7ToV8MigrationError(
    ...
    except ExecutableDevelopmentCoverageV7ToV8MigrationError:
        raise
    except (
...
@lru_cache(maxsize=1)
def executable_development_coverage_v7_to_v8_migration_config_bytes(
) -> bytes:
