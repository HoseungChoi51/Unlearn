"""Auditable migration evidence from development coverage v3 to v4.

The migration changes exactly one of 25 family values.  It proves that the
planned checksum parameter axes, tool policy, filesystem schema, output
contract, and capability tags are preserved during promotion; proves that all
other family values are identical; preserves the archive promotion as the
first history entry; and binds the new task set, discrimination digest, and
eleventh source-registry commitment.
"""
...
COVERAGE_V3_TO_V4_MIGRATION_SCHEMA_VERSION: Final[str] = "1.0.0"
COVERAGE_V3_TO_V4_MIGRATION_VERSION: Final[str] = "1.0.0"
COVERAGE_V3_TO_V4_MIGRATION_CONFIG_RELATIVE_PATH: Final[str] = (
    "configs/executable-method-development-coverage-v3-to-v4-migration.json"
)
MAXIMUM_COVERAGE_V3_TO_V4_MIGRATION_CONFIG_BYTES: Final[int] = 64 * 1024

FROZEN_V3_CHECKSUM_FAMILY_SHA256: Final[str] = (
    "8261a719e2afa702509ca16656760eed89f55c121667db0531a94168476f1bf9"
)
FROZEN_V4_CHECKSUM_FAMILY_SHA256: Final[str] = (
    "3541ee226666eb2fa69182980571ec6d315267d4bb9166bb247d6740050221bb"
)
FROZEN_COVERAGE_V3_TO_V4_MIGRATION_SHA256: Final[str] = (
...
MIGRATION_REASON_CODES: Final[tuple[str, ...]] = (
    ...
    "checksum-repair-discrimination",
    "eleventh-tranche-integration",
    "archive-promotion-history-preserved",
)
...
class ExecutableDevelopmentCoverageV3ToV4MigrationError(ValueError):
    """Raised when the v3-to-v4 migration evidence is not exact."""
...
class ExecutableDevelopmentCoverageV3ToV4Migration:
    ...
    preserved_promotion_history: tuple[
        coverage_v4.CoveragePromotionEvidence, ...
    ]
    promotion_evidence: coverage_v4.CoveragePromotionEvidence
    ...
    schema_version: str = COVERAGE_V3_TO_V4_MIGRATION_SCHEMA_VERSION
    migration_version: str = COVERAGE_V3_TO_V4_MIGRATION_VERSION
    ...
    def __post_init__(self) -> None:
        validate_executable_development_coverage_v3_to_v4_migration(self)
...
def _migration_record(
    migration: ExecutableDevelopmentCoverageV3ToV4Migration,
) -> dict[str, object]:
    return {
        "schema_version": migration.schema_version,
        "migration_version": migration.migration_version,
        "record_type": (
            "cbds.executable-method-development-coverage-v3-to-v4-migration"
        ),
        "from": {
            ...
            "config_bytes_sha256": (
                coverage_v4.PREDECESSOR_CONFIG_BYTES_SHA256
            ),
            "config_byte_count": (
                coverage_v4.PREDECESSOR_CONFIG_BYTE_COUNT
            ),
            "git_commit": coverage_v4.PREDECESSOR_GIT_COMMIT,
        },
        "to": {
        ...
        "changed_family_id": "checksum-repair-plan",
...
def compute_executable_development_coverage_v3_to_v4_migration_sha256(
    record: object,
) -> str:
    if type(record) is not dict:
        raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
...
def _migration_component_snapshot_bytes() -> bytes:
    ...
    old_coverage = coverage_v3.build_executable_development_coverage_v3()
    new_coverage = coverage_v4.build_executable_development_coverage_v4()
    if (
        old_coverage.coverage_sha256
        != coverage_v4.PREDECESSOR_COVERAGE_SHA256
        or new_coverage.coverage_sha256
        != coverage_v4.FROZEN_COVERAGE_V4_SHA256
        ...
        or len(new_coverage.promotion_history) != 2
        or new_coverage.promotion_history[0].to_record()
        != old_coverage.promotion_evidence.to_record()
    ):
        raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
            "live v3/v4 chain is not an exact append-only migration"
        )
    index = coverage_v4.CHECKSUM_FAMILY_INDEX
    ...
    if (
        ...
        raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
            "checksum family promotion did not preserve its planned contract"
        )
    unchanged: list[str] = []
    for family_index, (old, new) in enumerate(
        ...
        if family_index == index:
            if old == new:
                raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
                    "changed checksum family is unexpectedly identical"
                )
            continue
        if old != new:
            raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
    ...
    return canonical_json_bytes(
        {
            ...
            "preserved_promotion_history": [
                new_coverage.promotion_history[0].to_record()
            ],
            "promotion_evidence": (
...
def _migration_components() -> tuple[
    CoverageFamily,
    CoverageFamily,
    tuple[coverage_v4.CoveragePromotionEvidence, ...],
    coverage_v4.CoveragePromotionEvidence,
    SourceRegistryCommitment,
    tuple[str, ...],
]:
    ...
    try:
        ...
        if type(history_value) is not list or len(history_value) != 1:
            raise TypeError("preserved history must contain one entry")
        history = (
            coverage_v4._promotion_from_record(history_value[0]),
        )
        promotion = coverage_v4._promotion_from_record(
...
def validate_executable_development_coverage_v3_to_v4_migration(
    migration: ExecutableDevelopmentCoverageV3ToV4Migration,
) -> None:
    if (
        type(migration)
        is not ExecutableDevelopmentCoverageV3ToV4Migration
    ):
        raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
    ...
    if (
        ...
        != COVERAGE_V3_TO_V4_MIGRATION_SCHEMA_VERSION
        or migration.migration_version
        != COVERAGE_V3_TO_V4_MIGRATION_VERSION
    ...
    if (
        ...
        or any(
            type(item) is not coverage_v4.CoveragePromotionEvidence
            for item in migration.preserved_promotion_history
        )
        or type(migration.promotion_evidence)
        is not coverage_v4.CoveragePromotionEvidence
        ...
        raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
            "migration identities differ from live v3/v4 evidence"
        )
    for evidence in migration.preserved_promotion_history:
    ...
    if (
        ...
        != FROZEN_COVERAGE_V3_TO_V4_MIGRATION_SHA256
        or migration.migration_sha256
        != compute_executable_development_coverage_v3_to_v4_migration_sha256(
            record
        )
    ):
        raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
...
def build_executable_development_coverage_v3_to_v4_migration(
) -> ExecutableDevelopmentCoverageV3ToV4Migration:
    old, new, history, promotion, source, unchanged = (
        _migration_components()
    )
    provisional = (
        ExecutableDevelopmentCoverageV3ToV4Migration.__new__(
            ExecutableDevelopmentCoverageV3ToV4Migration
        )
    )
    values: dict[str, object] = {
        ...
        "to_coverage_sha256": coverage_v4.FROZEN_COVERAGE_V4_SHA256,
        ...
        "migration_version": COVERAGE_V3_TO_V4_MIGRATION_VERSION,
    ...
    return ExecutableDevelopmentCoverageV3ToV4Migration(
        from_coverage_sha256=(
            coverage_v4.PREDECESSOR_COVERAGE_SHA256
        ),
        to_coverage_sha256=coverage_v4.FROZEN_COVERAGE_V4_SHA256,
...
def load_executable_development_coverage_v3_to_v4_migration(
    ...
    try:
        payload = coverage_v4._read_stable_regular(
            source,
            MAXIMUM_COVERAGE_V3_TO_V4_MIGRATION_CONFIG_BYTES,
        )
    except coverage_v4.ExecutableDevelopmentCoverageV4Error as exc:
        raise ExecutableDevelopmentCoverageV3ToV4MigrationError(
    ...
    except ExecutableDevelopmentCoverageV3ToV4MigrationError:
        raise
    except (
...
@lru_cache(maxsize=1)
def executable_development_coverage_v3_to_v4_migration_config_bytes(
) -> bytes:
    """Return cached immutable canonical bytes for publication."""
