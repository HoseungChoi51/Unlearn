"""Additive registry for the pipefail-atomic-report development family.

The first four executable-static registries remain immutable.  This module
binds the exact family-local pipefail/atomic-report contract as a fifth
20-task addition
...
FIFTH_TRANCHE_REGISTRY_SCHEMA_VERSION: Final[str] = "1.0.0"
FIFTH_TRANCHE_REGISTRY_VERSION: Final[str] = "1.0.0"
FIFTH_TRANCHE_ADDED_TASK_COUNT: Final[int] = 20
FIFTH_TRANCHE_CUMULATIVE_TASK_COUNT: Final[int] = 280
...
FIFTH_TRANCHE_FAMILY_ORDER: Final[tuple[str, ...]] = (
    "pipefail-atomic-report",
)

FifthTrancheTask: TypeAlias = PipefailAtomicReportTask
_SHA256_RE: Final[re.Pattern[str]] = re.compile(r"[0-9a-f]{64}\Z")


class FifthTrancheRegistryError(ValueError):
    """Raised when the fifth additive registry is not reproducible."""
...
def build_fifth_tranche_added_tasks() -> tuple[FifthTrancheTask, ...]:
...
def _validate_added_tasks(
    tasks: object,
) -> tuple[FifthTrancheTask, ...]:
    if (
        type(tasks) is not tuple
        or len(tasks) != FIFTH_TRANCHE_ADDED_TASK_COUNT
        or any(type(task) is not PipefailAtomicReportTask for task in tasks)
    ):
        raise FifthTrancheRegistryError(
            "fifth tranche requires exactly 20 exact PipefailAtomicReportTask values"
        )
    selected = tasks
    try:
        ...
        raise FifthTrancheRegistryError(
            "fifth-tranche task validation failed"
        ) from exc

    expected_grid = tuple(
        (pipeline_shape, failure_commit_policy)
        for pipeline_shape in PIPEFAIL_ATOMIC_REPORT_PIPELINE_SHAPES
        for failure_commit_policy in (
            PIPEFAIL_ATOMIC_REPORT_FAILURE_COMMIT_POLICIES
        )
    )
    observed_grid = tuple(
    ...
    if observed_grid != expected_grid:
        raise FifthTrancheRegistryError(
            "fifth-tranche parameter grid is incomplete or out of order"
        )
    if (
        ...
        raise FifthTrancheRegistryError(
            "fifth-tranche task identities are not unique"
        )
    return selected


def _registry_payload(
    tasks: tuple[FifthTrancheTask, ...],
) -> dict[str, object]:
    selected = _validate_added_tasks(tasks)
    return {
        "schema_version": FIFTH_TRANCHE_REGISTRY_SCHEMA_VERSION,
        "registry_version": FIFTH_TRANCHE_REGISTRY_VERSION,
        "record_type": "cbds.executable-static-fifth-tranche-registry",
        "base_added_registry_sha256": FROZEN_FOURTH_ADDED_REGISTRY_SHA256,
...
def compute_fifth_tranche_registry_sha256(
    tasks: tuple[FifthTrancheTask, ...],
) -> str:
    return domain_sha256(
        "cbds.executable-static.fifth-tranche-registry.v1",
        _registry_payload(tasks),
    )


def compute_fifth_tranche_cumulative_suite_sha256(
    tasks: tuple[FifthTrancheTask, ...],
    registry_sha256: str,
) -> str:
    _validate_added_tasks(tasks)
    if not _is_sha256(registry_sha256):
        raise FifthTrancheRegistryError(
            "fifth-tranche registry digest is invalid"
        )
    if registry_sha256 != compute_fifth_tranche_registry_sha256(tasks):
        raise FifthTrancheRegistryError(
            "registry digest does not bind the fifth-tranche tasks"
        )
    return domain_sha256(
        "cbds.executable-static.fifth-tranche-cumulative-suite.v1",
        {
            "base_cumulative_suite_sha256": (
                FROZEN_FOURTH_CUMULATIVE_SUITE_SHA256
            ),
...
class FifthTrancheTaskRegistry:
    added_tasks: tuple[FifthTrancheTask, ...]
    registry_sha256: str
    cumulative_suite_sha256: str
    schema_version: str = FIFTH_TRANCHE_REGISTRY_SCHEMA_VERSION
    registry_version: str = FIFTH_TRANCHE_REGISTRY_VERSION
    base_added_registry_sha256: str = FROZEN_FOURTH_ADDED_REGISTRY_SHA256
    ...
    def __post_init__(self) -> None:
        validate_fifth_tranche_task_registry(self)
...
def validate_fifth_tranche_task_registry(
    ...
    if (
        ...
        or registry.schema_version != FIFTH_TRANCHE_REGISTRY_SCHEMA_VERSION
        or type(registry.registry_version) is not str
        or registry.registry_version != FIFTH_TRANCHE_REGISTRY_VERSION
        ...
        raise FifthTrancheRegistryError(
            "fifth-tranche registry metadata is invalid"
        )
    tasks = _validate_added_tasks(registry.added_tasks)
    expected_registry = compute_fifth_tranche_registry_sha256(tasks)
    if registry.registry_sha256 != expected_registry:
        raise FifthTrancheRegistryError(
            "fifth-tranche registry digest is invalid"
        )
    expected_suite = compute_fifth_tranche_cumulative_suite_sha256(
        tasks, expected_registry
    )
    if registry.cumulative_suite_sha256 != expected_suite:
        raise FifthTrancheRegistryError(
            "fifth-tranche cumulative suite digest is invalid"
        )
...
def _validate_live_base_and_global_uniqueness(
    ...
    if (
        len(all_tasks) != FIFTH_TRANCHE_CUMULATIVE_TASK_COUNT
        ...
        raise FifthTrancheRegistryError(
            "fifth-tranche tasks collide with a frozen predecessor"
        )


def build_fifth_tranche_task_registry() -> FifthTrancheTaskRegistry:
    tasks = build_fifth_tranche_added_tasks()
    _validate_live_base_and_global_uniqueness(tasks)
    registry_sha256 = compute_fifth_tranche_registry_sha256(tasks)
    cumulative_suite_sha256 = compute_fifth_tranche_cumulative_suite_sha256(
        tasks, registry_sha256
    )
    return FifthTrancheTaskRegistry(
        added_tasks=tasks,
...
__all__ = [
    ...
    "FIFTH_TRANCHE_REGISTRY_SCHEMA_VERSION",
    "FIFTH_TRANCHE_REGISTRY_VERSION",
