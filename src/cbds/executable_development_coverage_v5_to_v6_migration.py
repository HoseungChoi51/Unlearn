"""Auditable migration evidence from development coverage v5 to v6.

The migration changes exactly one of 25 family values.  It proves that the
planned nested-JSON schema-migration contract is preserved during promotion,
that all other family values are identical, and that the source and promotion
...
COVERAGE_V5_TO_V6_MIGRATION_SCHEMA_VERSION: Final[str] = "1.0.0"
COVERAGE_V5_TO_V6_MIGRATION_VERSION: Final[str] = "1.0.0"
COVERAGE_V5_TO_V6_MIGRATION_CONFIG_RELATIVE_PATH: Final[str] = (
    "configs/executable-method-development-coverage-v5-to-v6-migration.json"
)
MAXIMUM_COVERAGE_V5_TO_V6_MIGRATION_CONFIG_BYTES: Final[int] = 64 * 1024
FROZEN_COVERAGE_V5_TO_V6_MIGRATION_SHA256: Final[str] = (
    "5c345bc6860f5c9ff70dba656d3cc1204acb705a0d2c4526b4031364313d7e90"
)

FROZEN_V5_NESTED_JSON_FAMILY_SHA256: Final[str] = (
    "a2d00fd23b913baf301a992340d1373352f8f4cd95a954922af14eecaced5db9"
)
FROZEN_V6_NESTED_JSON_FAMILY_SHA256: Final[str] = (
    "cee48b9413a501b7976d713e878cd4ca4a621cbb568242e6b82e2678da530fc4"
)
MIGRATION_REASON_CODES: Final[tuple[str, ...]] = (
    "planned-contract-preserved",
    "canonical-grid-integration",
    "nested-json-schema-migration-discrimination",
    "thirteenth-tranche-integration",
...
class ExecutableDevelopmentCoverageV5ToV6MigrationError(ValueError):
    """Raised when the v5-to-v6 migration evidence is not exact."""
...
class ExecutableDevelopmentCoverageV5ToV6Migration:
    ...
    preserved_promotion_history: tuple[
        coverage_v6.CoveragePromotionEvidence, ...
    ]
    promotion_evidence: coverage_v6.CoveragePromotionEvidence
    ...
    schema_version: str = COVERAGE_V5_TO_V6_MIGRATION_SCHEMA_VERSION
    migration_version: str = COVERAGE_V5_TO_V6_MIGRATION_VERSION
    ...
    def __post_init__(self) -> None:
        validate_executable_development_coverage_v5_to_v6_migration(self)
...
def _migration_record(
    migration: ExecutableDevelopmentCoverageV5ToV6Migration,
) -> dict[str, object]:
    return {
        "schema_version": migration.schema_version,
        "migration_version": migration.migration_version,
        "record_type": (
            "cbds.executable-method-development-coverage-v5-to-v6-migration"
        ),
        "from": {
            ...
            "config_bytes_sha256": (
                coverage_v6.PREDECESSOR_CONFIG_BYTES_SHA256
            ),
            "config_byte_count": coverage_v6.PREDECESSOR_CONFIG_BYTE_COUNT,
            "git_commit": coverage_v6.PREDECESSOR_GIT_COMMIT,
        },
        "to": {
        ...
        "changed_family_id": "nested-json-schema-migration",
...
def compute_executable_development_coverage_v5_to_v6_migration_sha256(
    record: object,
) -> str:
    if type(record) is not dict:
        raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
...
def _migration_component_snapshot_bytes() -> bytes:
    ...
    old_coverage = coverage_v5.build_executable_development_coverage_v5()
    new_coverage = coverage_v6.build_executable_development_coverage_v6()
    if (
        old_coverage.coverage_sha256
        != coverage_v6.PREDECESSOR_COVERAGE_SHA256
        or new_coverage.coverage_sha256
        != coverage_v6.FROZEN_COVERAGE_V6_SHA256
        ...
        or len(new_coverage.promotion_history) != 4
        or [
            item.to_record()
        ...
        raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
            "live v5/v6 chain is not an exact append-only migration"
        )
    index = coverage_v6.NESTED_JSON_FAMILY_INDEX
    ...
    if (
        ...
        raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
            "nested-JSON family promotion did not preserve its planned contract"
        )
    unchanged: list[str] = []
    for family_index, (old, new) in enumerate(
        ...
        if family_index == index:
            if old == new:
                raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
                    "changed nested-JSON family is unexpectedly identical"
                )
            continue
        if old != new:
            raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
...
def _migration_components() -> tuple[
    ...
    try:
        ...
        if type(history_value) is not list or len(history_value) != 3:
            raise TypeError("preserved history must contain three entries")
        history = tuple(
            coverage_v6._promotion_from_record(item)
            for item in history_value
        )
        promotion = coverage_v6._promotion_from_record(
...
def validate_executable_development_coverage_v5_to_v6_migration(
    migration: ExecutableDevelopmentCoverageV5ToV6Migration,
) -> None:
    if (
        type(migration)
        is not ExecutableDevelopmentCoverageV5ToV6Migration
    ):
        raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
    ...
    if (
        ...
        != COVERAGE_V5_TO_V6_MIGRATION_SCHEMA_VERSION
        or type(migration.migration_version) is not str
        or migration.migration_version
        != COVERAGE_V5_TO_V6_MIGRATION_VERSION
    ...
    if (
        ...
        or any(
            type(item) is not coverage_v6.CoveragePromotionEvidence
            for item in migration.preserved_promotion_history
        )
        or type(migration.promotion_evidence)
        is not coverage_v6.CoveragePromotionEvidence
        ...
        raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
            "migration identities differ from live v5/v6 evidence"
        )
    for evidence in migration.preserved_promotion_history:
    ...
    if (
        ...
        != FROZEN_COVERAGE_V5_TO_V6_MIGRATION_SHA256
        or migration.migration_sha256
        != compute_executable_development_coverage_v5_to_v6_migration_sha256(
            record
        )
    ):
        raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
...
def build_executable_development_coverage_v5_to_v6_migration(
) -> ExecutableDevelopmentCoverageV5ToV6Migration:
    old, new, history, promotion, source, unchanged = (
        _migration_components()
    )
    provisional = ExecutableDevelopmentCoverageV5ToV6Migration.__new__(
        ExecutableDevelopmentCoverageV5ToV6Migration
    )
    values: dict[str, object] = {
        "from_coverage_sha256": coverage_v6.PREDECESSOR_COVERAGE_SHA256,
        "to_coverage_sha256": coverage_v6.FROZEN_COVERAGE_V6_SHA256,
        ...
        "schema_version": COVERAGE_V5_TO_V6_MIGRATION_SCHEMA_VERSION,
        "migration_version": COVERAGE_V5_TO_V6_MIGRATION_VERSION,
    ...
    return ExecutableDevelopmentCoverageV5ToV6Migration(
        from_coverage_sha256=coverage_v6.PREDECESSOR_COVERAGE_SHA256,
        to_coverage_sha256=coverage_v6.FROZEN_COVERAGE_V6_SHA256,
        old_family_sha256=old.family_sha256,
        new_family_sha256=new.family_sha256,
...
def load_executable_development_coverage_v5_to_v6_migration(
    ...
    try:
        payload = coverage_v6._read_stable_regular(
            source,
            MAXIMUM_COVERAGE_V5_TO_V6_MIGRATION_CONFIG_BYTES,
        )
    except coverage_v6.ExecutableDevelopmentCoverageV6Error as exc:
        raise ExecutableDevelopmentCoverageV5ToV6MigrationError(
    ...
    except ExecutableDevelopmentCoverageV5ToV6MigrationError:
        raise
    except (
...
@lru_cache(maxsize=1)
def executable_development_coverage_v5_to_v6_migration_config_bytes(
) -> bytes:
