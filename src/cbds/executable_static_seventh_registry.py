"""Additive registry for the case-routed-batch-transform family.

The first six executable-static registries remain immutable.  This module
binds the exact family-local case-routing contract as a seventh 20-task
...
SEVENTH_TRANCHE_REGISTRY_SCHEMA_VERSION: Final[str] = "1.0.0"
SEVENTH_TRANCHE_REGISTRY_VERSION: Final[str] = "1.0.0"
SEVENTH_TRANCHE_ADDED_TASK_COUNT: Final[int] = 20
SEVENTH_TRANCHE_CUMULATIVE_TASK_COUNT: Final[int] = 320
...
SEVENTH_TRANCHE_FAMILY_ORDER: Final[tuple[str, ...]] = (
    "case-routed-batch-transform",
)

SeventhTrancheTask: TypeAlias = CaseRoutedBatchTransformTask
_SHA256_RE: Final[re.Pattern[str]] = re.compile(r"[0-9a-f]{64}\Z")


class SeventhTrancheRegistryError(ValueError):
    """Raised when the seventh additive registry is not reproducible."""
...
def build_seventh_tranche_added_tasks() -> tuple[SeventhTrancheTask, ...]:
...
def _validate_added_tasks(tasks: object) -> tuple[SeventhTrancheTask, ...]:
    if (
        type(tasks) is not tuple
        or len(tasks) != SEVENTH_TRANCHE_ADDED_TASK_COUNT
        or any(type(task) is not CaseRoutedBatchTransformTask for task in tasks)
    ):
        raise SeventhTrancheRegistryError(
            "seventh tranche requires exactly 20 exact "
            "CaseRoutedBatchTransformTask values"
        )
    selected = tasks
    try:
        ...
        raise SeventhTrancheRegistryError(
            "seventh-tranche task validation failed"
        ) from exc
    ...
    if observed_grid != expected_grid:
        raise SeventhTrancheRegistryError(
            "seventh-tranche parameter grid is incomplete or out of order"
        )
    if (
        ...
        raise SeventhTrancheRegistryError(
            "seventh-tranche task identities are not unique"
        )
    return selected


def _registry_payload(
    tasks: tuple[SeventhTrancheTask, ...],
) -> dict[str, object]:
    selected = _validate_added_tasks(tasks)
    return {
        "schema_version": SEVENTH_TRANCHE_REGISTRY_SCHEMA_VERSION,
        "registry_version": SEVENTH_TRANCHE_REGISTRY_VERSION,
        "record_type": "cbds.executable-static-seventh-tranche-registry",
        "base_added_registry_sha256": FROZEN_SIXTH_ADDED_REGISTRY_SHA256,
...
def compute_seventh_tranche_registry_sha256(
    tasks: tuple[SeventhTrancheTask, ...],
) -> str:
    return domain_sha256(
        "cbds.executable-static.seventh-tranche-registry.v1",
        _registry_payload(tasks),
    )


def compute_seventh_tranche_cumulative_suite_sha256(
    tasks: tuple[SeventhTrancheTask, ...],
    registry_sha256: str,
) -> str:
    _validate_added_tasks(tasks)
    if not _is_sha256(registry_sha256):
        raise SeventhTrancheRegistryError(
            "seventh-tranche registry digest is invalid"
        )
    if registry_sha256 != compute_seventh_tranche_registry_sha256(tasks):
        raise SeventhTrancheRegistryError(
            "registry digest does not bind the seventh-tranche tasks"
        )
    return domain_sha256(
        "cbds.executable-static.seventh-tranche-cumulative-suite.v1",
        {
            "base_cumulative_suite_sha256": (
                FROZEN_SIXTH_CUMULATIVE_SUITE_SHA256
            ),
...
class SeventhTrancheTaskRegistry:
    added_tasks: tuple[SeventhTrancheTask, ...]
    registry_sha256: str
    cumulative_suite_sha256: str
    schema_version: str = SEVENTH_TRANCHE_REGISTRY_SCHEMA_VERSION
    registry_version: str = SEVENTH_TRANCHE_REGISTRY_VERSION
    base_added_registry_sha256: str = FROZEN_SIXTH_ADDED_REGISTRY_SHA256
    ...
    def __post_init__(self) -> None:
        validate_seventh_tranche_task_registry(self)
...
def validate_seventh_tranche_task_registry(
    ...
    if (
        ...
        or registry.schema_version != SEVENTH_TRANCHE_REGISTRY_SCHEMA_VERSION
        or type(registry.registry_version) is not str
        or registry.registry_version != SEVENTH_TRANCHE_REGISTRY_VERSION
        ...
        raise SeventhTrancheRegistryError(
            "seventh-tranche registry metadata is invalid"
        )
    tasks = _validate_added_tasks(registry.added_tasks)
    expected_registry = compute_seventh_tranche_registry_sha256(tasks)
    if registry.registry_sha256 != expected_registry:
        raise SeventhTrancheRegistryError(
            "seventh-tranche registry digest is invalid"
        )
    expected_suite = compute_seventh_tranche_cumulative_suite_sha256(
        tasks, expected_registry
    )
    if registry.cumulative_suite_sha256 != expected_suite:
        raise SeventhTrancheRegistryError(
            "seventh-tranche cumulative suite digest is invalid"
        )
...
def _validate_live_base_and_global_uniqueness(
    ...
    if (
        len(all_tasks) != SEVENTH_TRANCHE_CUMULATIVE_TASK_COUNT
        ...
        raise SeventhTrancheRegistryError(
            "seventh-tranche tasks collide with a frozen predecessor"
        )


def build_seventh_tranche_task_registry() -> SeventhTrancheTaskRegistry:
    tasks = build_seventh_tranche_added_tasks()
    _validate_live_base_and_global_uniqueness(tasks)
    registry_sha256 = compute_seventh_tranche_registry_sha256(tasks)
    cumulative_suite_sha256 = compute_seventh_tranche_cumulative_suite_sha256(
        tasks, registry_sha256
    )
    return SeventhTrancheTaskRegistry(
        added_tasks=tasks,
...
__all__ = [
    ...
    "SEVENTH_TRANCHE_REGISTRY_SCHEMA_VERSION",
    "SEVENTH_TRANCHE_REGISTRY_VERSION",
