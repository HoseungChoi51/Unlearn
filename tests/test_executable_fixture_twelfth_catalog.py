...
import cbds.executable_fixture_twelfth_catalog as twelfth_catalog  # noqa: E402
...
from cbds.executable_fixture_twelfth_catalog import (  # noqa: E402
...
EXPECTED_TWELFTH_CATALOG_SHA256 = (
...
class TwelfthTrancheFixtureCatalogTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        captured: dict[str, object] = {}
        real_task_builder = build_eleventh_prefix_task_evidence
        real_fixture_builder = build_eleventh_prefix_fixture_evidence
        ...
        with ExitStack() as stack:
            ...
            cls.catalog = build_twelfth_tranche_fixture_catalog()
    ...
    def test_exact_counts_types_and_task_major_profile_minor_order(self) -> None:
        ...
        self.assertEqual(
            len(bundles), TWELFTH_TRANCHE_ADDED_FIXTURE_COUNT
        )
        self.assertEqual(TWELFTH_TRANCHE_ADDED_FIXTURE_COUNT, 100)
        self.assertEqual(TWELFTH_TRANCHE_FIXTURE_COUNT, 100)
        self.assertEqual(
            TWELFTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT, 2_100
        )
        self.assertEqual(TWELFTH_TRANCHE_PROFILE_COUNT, 5)
        self.assertTrue(
            all(
        ...
        self.assertTrue(
            all(
                type(bundle) is JsonlCsvEnrichmentComposeFixtureBundle
                for bundle in bundles
            )
        )
        for index, bundle in enumerate(bundles):
            task = tasks[index // TWELFTH_TRANCHE_PROFILE_COUNT]
    ...
    def test_exact_through_eleventh_prefix_is_built_once_and_shared(
        self,
    ) -> None:
    ...
    def test_fixture_identities_are_globally_unique_across_2100(self) -> None:
        all_bundles = (
    ...
    def test_catalog_digest_is_hash_only_and_nonauthorizing(self) -> None:
        ...
        self.assertTrue(verify_twelfth_tranche_fixture_catalog(self.catalog))
        self.assertFalse(_contains_bytes(self.record))
        self.assertEqual(
            self.record["record_type"],
            "cbds.executable-fixture-twelfth-tranche-catalog",
        )
        self.assertEqual(self.record["base_cumulative_task_count"], 400)
        self.assertEqual(self.record["added_task_count"], 20)
        self.assertEqual(self.record["cumulative_task_count"], 420)
        self.assertEqual(
            self.record["base_cumulative_fixture_count"], 2_000
        )
        self.assertEqual(self.record["added_fixture_count"], 100)
        self.assertEqual(
            self.record["cumulative_fixture_count"], 2_100
        )
        self.assertEqual(
    ...
    def test_local_rebuild_is_deterministic_with_fresh_bundle_ownership(
        ...
        rebuilt = build_twelfth_tranche_fixture_catalog_local(
            self.catalog.registry
        )
    ...
    def test_tampering_wrong_types_and_forged_prefix_fail_closed(self) -> None:
        self.assertFalse(verify_twelfth_tranche_fixture_catalog(object()))
        with self.assertRaises(TwelfthTrancheFixtureCatalogError):
            replace(
                self.catalog,
        ...
        with self.assertRaises(TwelfthTrancheFixtureCatalogError):
            validate_twelfth_tranche_fixture_catalog(reordered)
        ...
        with self.assertRaises(TwelfthTrancheFixtureCatalogError):
            twelfth_catalog._validate_live_base_and_global_uniqueness(
                self.catalog.registry,
                self.catalog.bundles,
                forged,
            )
