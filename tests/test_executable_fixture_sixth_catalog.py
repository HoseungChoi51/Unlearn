...
class SixthTrancheFixtureCatalogTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.catalog = build_sixth_tranche_fixture_catalog()
    ...
    def assert_catalog_invalid(self, catalog: object) -> None:
        self.assertFalse(verify_sixth_tranche_fixture_catalog(catalog))
        with self.assertRaises(
            (SixthTrancheFixtureCatalogError, TypeError, ValueError)
        ):
            validate_sixth_tranche_fixture_catalog(  # type: ignore[arg-type]
                catalog
            )
    ...
    def test_exact_types_are_task_major_profile_minor(self) -> None:
        ...
        self.assertEqual(len(bundles), SIXTH_TRANCHE_ADDED_FIXTURE_COUNT)
        self.assertEqual(SIXTH_TRANCHE_ADDED_FIXTURE_COUNT, 100)
        self.assertEqual(SIXTH_TRANCHE_FIXTURE_COUNT, 100)
        self.assertEqual(SIXTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT, 1_500)
        self.assertEqual(SIXTH_TRANCHE_PROFILE_COUNT, 5)
        self.assertTrue(
            all(type(task) is BoundedRetryStateMachineTask for task in tasks)
        )
        self.assertTrue(
            all(
                type(bundle) is BoundedRetryStateMachineFixtureBundle
        ...
        for index, bundle in enumerate(bundles):
            task = tasks[index // SIXTH_TRANCHE_PROFILE_COUNT]
    ...
    def test_rebuild_is_deterministic_hash_bound_and_never_executes(self) -> None:
        with mock.patch.object(
            ...
            rebuilt = build_sixth_tranche_fixture_catalog()
        ...
        self.assertEqual(
            rebuilt.catalog_sha256, EXPECTED_SIXTH_CATALOG_SHA256
        )
        self.assertEqual(
    ...
    def test_all_1500_fixture_identities_are_globally_unique(self) -> None:
        bundles = (
            *self.first.bundles,
            *self.second.bundles,
            *self.third.bundles,
        ...
        self.assertEqual(len(bundles), SIXTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT)
        self.assertEqual(
    ...
    def test_projection_binds_generator_and_is_hash_only(self) -> None:
        ...
        self.assertEqual(
            record["record_type"],
            "cbds.executable-fixture-sixth-tranche-catalog",
        )
        self.assertEqual(record["base_cumulative_task_count"], 280)
        self.assertEqual(record["added_task_count"], 20)
        self.assertEqual(record["cumulative_task_count"], 300)
        self.assertEqual(record["base_cumulative_fixture_count"], 1_400)
        self.assertEqual(record["added_fixture_count"], 100)
        self.assertEqual(record["cumulative_fixture_count"], 1_500)
        self.assertEqual(record["profiles_per_task"], 5)
        self.assertEqual(
            record["family_task_counts"],
            {BOUNDED_RETRY_STATE_MACHINE_FAMILY_ID: 20},
        )
        self.assertEqual(
    ...
    def test_order_nested_type_authority_and_spoof_mutations_fail(self) -> None:
        ...
        object.__setattr__(
            wrong_bundle_type,
            "bundles",
            (self.fifth.bundles[0], *self.catalog.bundles[1:]),
        )
