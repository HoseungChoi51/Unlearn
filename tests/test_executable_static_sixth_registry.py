...
EXPECTED_SIXTH_REGISTRY_SHA256 = (
    "14280b3cbc8a96c919a57a325b5795c381cba86b2a31934f7069821b7ff4e3c4"
)
EXPECTED_SIXTH_CUMULATIVE_SUITE_SHA256 = (
...
class SixthTrancheTaskRegistryTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.registry = build_sixth_tranche_task_registry()
    ...
    def assert_registry_invalid(self, registry: object) -> None:
        with self.assertRaises(
            (SixthTrancheRegistryError, TypeError, ValueError)
        ):
            validate_sixth_tranche_task_registry(  # type: ignore[arg-type]
                registry
            )

    def test_grid_has_exact_type_order_and_unique_identities(self) -> None:
        tasks = self.registry.added_tasks
        self.assertEqual(len(tasks), SIXTH_TRANCHE_ADDED_TASK_COUNT)
        self.assertEqual(SIXTH_TRANCHE_ADDED_TASK_COUNT, 20)
        self.assertEqual(SIXTH_TRANCHE_CUMULATIVE_TASK_COUNT, 300)
        self.assertEqual(
            SIXTH_TRANCHE_FAMILY_ORDER,
    ...
    def test_rebuild_is_deterministic_and_new_hashes_are_frozen(self) -> None:
        self.assertEqual(
            build_sixth_tranche_added_tasks(), self.registry.added_tasks
        )
        rebuilt = build_sixth_tranche_task_registry()
        ...
        self.assertEqual(
            rebuilt.registry_sha256,
            compute_sixth_tranche_registry_sha256(rebuilt.added_tasks),
        )
        self.assertEqual(
            rebuilt.cumulative_suite_sha256,
            compute_sixth_tranche_cumulative_suite_sha256(
    ...
    def test_all_300_task_identities_are_globally_unique(self) -> None:
        tasks = (
            *self.first.tasks,
        ...
        self.assertEqual(len(tasks), SIXTH_TRANCHE_CUMULATIVE_TASK_COUNT)
    ...
    def test_hash_projection_is_public_unsealed_and_nonauthorizing(self) -> None:
        ...
        self.assertEqual(
            ...
            {"bounded-retry-state-machine": 20},
        )
    ...
    def test_mutations_wrong_types_and_hostile_strings_fail_closed(self) -> None:
        ...
        object.__setattr__(
            wrong_task_type,
            "added_tasks",
            (self.fifth.added_tasks[0], *self.registry.added_tasks[1:]),
        )
