...
EXPECTED_EIGHTH_CATALOG_SHA256 = (
...
class EighthTrancheFixtureCatalogTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.seventh = build_seventh_tranche_fixture_catalog()

    def assert_catalog_invalid(self, catalog: object) -> None:
        self.assertFalse(verify_eighth_tranche_fixture_catalog(catalog))
        with self.assertRaises(
            (EighthTrancheFixtureCatalogError, TypeError, ValueError)
        ):
            validate_eighth_tranche_fixture_catalog(  # type: ignore[arg-type]
                catalog
            )
    ...
    def test_exact_types_are_task_major_profile_minor(self) -> None:
        ...
        self.assertEqual(len(bundles), EIGHTH_TRANCHE_ADDED_FIXTURE_COUNT)
        self.assertEqual(EIGHTH_TRANCHE_ADDED_FIXTURE_COUNT, 100)
        self.assertEqual(EIGHTH_TRANCHE_FIXTURE_COUNT, 100)
        self.assertEqual(EIGHTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT, 1_700)
        self.assertEqual(EIGHTH_TRANCHE_PROFILE_COUNT, 5)
        self.assertTrue(
            all(type(task) is CollisionSafeBatchRenameTask for task in tasks)
        )
        self.assertTrue(
            all(
                type(bundle) is CollisionSafeBatchRenameFixtureBundle
        ...
        for index, bundle in enumerate(bundles):
            task = tasks[index // EIGHTH_TRANCHE_PROFILE_COUNT]
    ...
    def test_rebuild_is_deterministic_hash_bound_and_never_executes(self) -> None:
        with mock.patch.object(
            ...
            rebuilt = build_eighth_tranche_fixture_catalog()
    ...
    def test_all_predecessor_catalogs_are_live_and_unchanged(self) -> None:
        ...
        self.assertEqual(
            self.seventh.catalog_sha256, FROZEN_SEVENTH_CATALOG_SHA256
        )
        self.assertEqual(
            self.catalog.base_fixture_catalog_sha256,
            self.seventh.catalog_sha256,
        )
        self.assertEqual(
    ...
    def test_all_1700_fixture_identities_are_globally_unique(self) -> None:
        bundles = (
            *self.first.bundles,
            *self.second.bundles,
            *self.third.bundles,
            ...
            *self.fifth.bundles,
            *self.sixth.bundles,
            *self.seventh.bundles,
            *self.catalog.bundles,
        )
        self.assertEqual(len(bundles), EIGHTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT)
        self.assertEqual(
    ...
    def test_projection_binds_generator_and_is_hash_only(self) -> None:
        ...
        self.assertEqual(record["base_cumulative_fixture_count"], 1_600)
        self.assertEqual(record["added_fixture_count"], 100)
        self.assertEqual(record["cumulative_fixture_count"], 1_700)
        self.assertEqual(record["profiles_per_task"], 5)
        self.assertEqual(
            record["family_task_counts"],
            {COLLISION_SAFE_BATCH_RENAME_FAMILY_ID: 20},
        )
        self.assertEqual(
    ...
    def test_order_nested_type_authority_and_spoof_mutations_fail(self) -> None:
        ...
        object.__setattr__(
            wrong_bundle_type,
            "bundles",
            (self.seventh.bundles[0], *self.catalog.bundles[1:]),
        )
