...
class FourthTrancheFullCatalogMaterializationTests(unittest.TestCase):
    def test_all_100_bundles_reject_missing_and_corrupt_outputs(self) -> None:
        family_passes: dict[str, int] = {}
        profile_passes: dict[str, int] = {}
        missing_rejections = 0
        exact_acceptances = 0
        semantic_variant_acceptances = 0
        ...
        with mock.patch.object(
            ...
            validate_fourth_tranche_fixture_catalog(catalog)
            ...
            for index, bundle in enumerate(catalog.bundles):
                ...
                with self.subTest(
                    ...
                    with materialize_ustar_pack_fixture(
                        ...
                        trailing_zero_variant = output.content + (b"\0" * 512)
                        self.assertNotEqual(trailing_zero_variant, output.content)
                        self.assertLessEqual(
                            len(trailing_zero_variant),
                            USTAR_PACK_OUTPUT_MAXIMUM_BYTES,
                        )
                        _replace_output_descriptor_relative(
                            workspace,
                            output.path,
                            trailing_zero_variant,
                            output.mode,
                        )
                        self.assertTrue(
                            verify_ustar_pack_workspace(
                        ...
                        semantic_variant_acceptances += 1
        ...
        self.assertEqual(family_passes, {USTAR_PACK_FAMILY_ID: 100})
        self.assertEqual(
            profile_passes,
            {
        ...
        self.assertEqual(semantic_variant_acceptances, 100)
