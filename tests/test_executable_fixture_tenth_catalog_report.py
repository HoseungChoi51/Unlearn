...
SCRIPT_PATH = ROOT / "scripts" / "build_executable_tenth_tranche_catalog.py"
SCRIPT_SPEC = importlib.util.spec_from_file_location(
    "cbds_build_executable_tenth_tranche_catalog",
    SCRIPT_PATH,
)
if SCRIPT_SPEC is None or SCRIPT_SPEC.loader is None:
    raise RuntimeError("tenth-tranche report builder cannot be loaded")
...
REPORT = ROOT / "reports" / "executable-tenth-tranche" / "manifest.json"
EXPECTED_REPORT_SHA256 = (
    "02442d60bf7d7874016fc9d50857cd49f9d8e1342ece55a42d7c8afcd852f0fb"
)
EXPECTED_REPORT_LENGTH = 56_553
...
class TenthTrancheCatalogReportTests(unittest.TestCase):
    @classmethod
    ...
    def test_builder_serializes_only_central_bounded_projection(self) -> None:
        with mock.patch.object(
            ...
            first = REPORT_SCRIPT.canonical_tenth_tranche_catalog_bytes()
            second = REPORT_SCRIPT.canonical_tenth_tranche_catalog_bytes()
        self.assertEqual(central_builder.call_count, 2)
        self.assertEqual(first, self.payload)
        self.assertEqual(second, self.payload)
        self.assertLessEqual(
            len(first), REPORT_SCRIPT.TENTH_TRANCHE_REPORT_MAXIMUM_BYTES
        )
        parsed = json.loads(
    ...
    def test_authority_or_size_drift_fails_before_publication(self) -> None:
        ...
        with mock.patch.object(
            ...
            REPORT_SCRIPT.canonical_tenth_tranche_catalog_bytes()

        with tempfile.TemporaryDirectory() as temporary:
            report = Path(temporary) / "manifest.json"
            oversized = b"x" * (
                REPORT_SCRIPT.TENTH_TRANCHE_REPORT_MAXIMUM_BYTES + 1
            )
    ...
    def test_publication_is_idempotent_exact_and_no_replace(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                list(report.parent.glob(".cbds-tenth-*.tmp")), []
            )
            REPORT_SCRIPT.atomic_publish_noreplace(report, self.payload)
            second = report.lstat()
            self.assertEqual(
    ...
    def test_script_delegates_to_shared_hardened_primitives(self) -> None:
        path = Path("/tmp/cbds-tenth-report-delegation.json")
        with mock.patch.object(
            REPORT_SCRIPT,
        ...
        publisher.assert_called_once_with(
            path,
            b"{}\n",
            REPORT_SCRIPT.TENTH_TRANCHE_TEMPORARY_PREFIX,
        )
        with mock.patch.object(
            REPORT_SCRIPT,
        ...
        reader.assert_called_once_with(
            path,
            REPORT_SCRIPT.TENTH_TRANCHE_REPORT_MAXIMUM_BYTES,
        )
