...
SCRIPT_PATH = (
    ROOT / "scripts" / "build_executable_fourteenth_tranche_catalog.py"
)
SCRIPT_SPEC = importlib.util.spec_from_file_location(
    "cbds_build_executable_fourteenth_tranche_catalog",
    SCRIPT_PATH,
)
if SCRIPT_SPEC is None or SCRIPT_SPEC.loader is None:
    raise RuntimeError("fourteenth-tranche report builder cannot be loaded")
...
REPORT = (
    ROOT / "reports" / "executable-fourteenth-tranche" / "manifest.json"
)
EXPECTED_REPORT_SHA256 = (
    "731f3ff9d03befb25ee72a5ed7ea13a17cd30aedfe60cd0d84df9aed5276a490"
)
EXPECTED_REPORT_LENGTH = 56_419
...
class FourteenthTrancheCatalogReportTests(unittest.TestCase):
    @classmethod
    ...
    def test_builder_serializes_only_central_bounded_projection(self) -> None:
        with mock.patch.object(
            ...
            first = (
                REPORT_SCRIPT.canonical_fourteenth_tranche_catalog_bytes()
            )
            second = (
                REPORT_SCRIPT.canonical_fourteenth_tranche_catalog_bytes()
            )
        ...
        self.assertLessEqual(
            len(first),
            REPORT_SCRIPT.FOURTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES,
        )
        parsed = json.loads(
    ...
    def test_projection_is_hash_only_public_and_nonauthorizing(self) -> None:
        ...
        self.assertEqual(self.record["cumulative_fixture_count"], 2_300)
    ...
    def test_authority_or_size_drift_fails_before_publication(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            oversized = b"x" * (
                REPORT_SCRIPT.FOURTEENTH_TRANCHE_REPORT_MAXIMUM_BYTES + 1
            )
    ...
    def test_publication_is_idempotent_exact_and_no_replace(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                list(report.parent.glob(".cbds-fourteenth-*.tmp")),
                [],
            )
            REPORT_SCRIPT.atomic_publish_noreplace(report, self.payload)
            second = report.lstat()
    ...
    def test_check_mode_and_committed_artifact_are_exact(self) -> None:
        ...
        with mock.patch.object(
            REPORT_SCRIPT,
            "canonical_fourteenth_tranche_catalog_bytes",
    ...
    def test_script_delegates_to_shared_hardened_primitives(self) -> None:
        path = Path("/tmp/cbds-fourteenth-report-delegation.json")
        with mock.patch.object(
            REPORT_SCRIPT,
        ...
        publisher.assert_called_once_with(
            path,
            b"{}\n",
            REPORT_SCRIPT.FOURTEENTH_TRANCHE_TEMPORARY_PREFIX,
        )
        with mock.patch.object(
            REPORT_SCRIPT,
