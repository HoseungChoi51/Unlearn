...
SCRIPT_PATH = ROOT / "scripts" / "build_executable_sixth_tranche_catalog.py"
SCRIPT_SPEC = importlib.util.spec_from_file_location(
    "cbds_build_executable_sixth_tranche_catalog",
    SCRIPT_PATH,
)
if SCRIPT_SPEC is None or SCRIPT_SPEC.loader is None:
    raise RuntimeError("sixth-tranche report builder cannot be loaded")
...
REPORT = ROOT / "reports" / "executable-sixth-tranche" / "manifest.json"
...
class SixthTrancheCatalogReportTests(unittest.TestCase):
    @classmethod
    ...
    def test_builder_serializes_only_the_central_canonical_projection(self) -> None:
        with mock.patch.object(
            ...
            first = REPORT_SCRIPT.canonical_sixth_tranche_catalog_bytes()
            second = REPORT_SCRIPT.canonical_sixth_tranche_catalog_bytes()
    ...
    def test_projection_binds_hashes_and_contains_no_sensitive_bytes(self) -> None:
        ...
        self.assertEqual(self.record["cumulative_task_count"], 300)
        self.assertEqual(self.record["added_fixture_count"], 100)
        self.assertEqual(self.record["cumulative_fixture_count"], 1_500)
        ...
        self.assertEqual(
            generators[0]["family_id"], "bounded-retry-state-machine"
        )
    ...
    def test_atomic_publication_is_idempotent_and_never_replaces(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            report = Path(temporary) / "nested" / "sixth-catalog.json"
            ...
            with self.assertRaisesRegex(
                REPORT_SCRIPT.SixthTrancheCatalogPublicationError,
                "differs",
            ):
    ...
    def test_cli_uses_central_projection_and_checks_caller_path(self) -> None:
        with tempfile.TemporaryDirectory() as temporary, mock.patch.object(
            REPORT_SCRIPT,
            "canonical_sixth_tranche_catalog_bytes",
    ...
    def test_checked_manifest_has_frozen_byte_identity(self) -> None:
        self.assertTrue(
            REPORT.is_file(),
            "the frozen sixth-tranche manifest must be checked in",
        )
