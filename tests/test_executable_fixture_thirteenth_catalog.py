...
import cbds.executable_fixture_thirteenth_catalog as catalog_module  # noqa: E402
from cbds.executable_fixture_profiles import (  # noqa: E402
    PUBLIC_DEVELOPMENT_FIXTURE_PROFILES,
)
from cbds.executable_fixture_thirteenth_catalog import (  # noqa: E402
...
EXPECTED_THIRTEENTH_CATALOG_SHA256 = (
...
class ThirteenthTrancheFixtureCatalogTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        captured: dict[str, object] = {}
        real_task_builder = build_twelfth_prefix_task_evidence
        real_fixture_builder = build_twelfth_prefix_fixture_evidence
        ...
        with ExitStack() as stack:
            ...
            cls.catalog = build_thirteenth_tranche_fixture_catalog()
    ...
    def test_exact_counts_types_and_task_major_profile_minor_order(self) -> None:
        ...
        self.assertEqual(
            len(bundles), THIRTEENTH_TRANCHE_ADDED_FIXTURE_COUNT
        )
        self.assertEqual(THIRTEENTH_TRANCHE_ADDED_FIXTURE_COUNT, 100)
        self.assertEqual(THIRTEENTH_TRANCHE_FIXTURE_COUNT, 100)
        self.assertEqual(
            THIRTEENTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT, 2_200
        )
        self.assertEqual(THIRTEENTH_TRANCHE_PROFILE_COUNT, 5)
        self.assertTrue(
            all(
        ...
        self.assertTrue(
            all(
                type(bundle) is NestedJsonSchemaMigrationFixtureBundle
                for bundle in bundles
            )
        )
        for index, bundle in enumerate(bundles):
            task = tasks[index // THIRTEENTH_TRANCHE_PROFILE_COUNT]
    ...
    def test_fixture_identities_are_globally_unique_across_2200(self) -> None:
        all_tasks = (
        ...
        self.assertEqual(len(all_tasks), 440)
        self.assertEqual(
            len(all_bundles),
            THIRTEENTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT,
        )
        ...
        for index, bundle in enumerate(all_bundles):
            task = all_tasks[index // THIRTEENTH_TRANCHE_PROFILE_COUNT]
            profile_index = index % THIRTEENTH_TRANCHE_PROFILE_COUNT
            profile = PUBLIC_DEVELOPMENT_FIXTURE_PROFILES[profile_index]
            self.assertEqual(
    ...
    def test_catalog_digest_is_hash_only_and_nonauthorizing(self) -> None:
        ...
        self.assertTrue(
            verify_thirteenth_tranche_fixture_catalog(self.catalog)
        )
        ...
        self.assertEqual(self.record["base_cumulative_task_count"], 420)
        self.assertEqual(self.record["added_task_count"], 20)
        self.assertEqual(self.record["cumulative_task_count"], 440)
        self.assertEqual(
            self.record["base_cumulative_fixture_count"], 2_100
        )
        self.assertEqual(self.record["added_fixture_count"], 100)
        self.assertEqual(
            self.record["cumulative_fixture_count"], 2_200
        )
        self.assertEqual(
    ...
    def test_local_rebuild_is_deterministic_with_fresh_ownership(self) -> None:
        rebuilt = build_thirteenth_tranche_fixture_catalog_local(
            self.catalog.registry
        )
    ...
    def test_tampering_wrong_types_and_forged_prefix_fail_closed(self) -> None:
        self.assertFalse(
            verify_thirteenth_tranche_fixture_catalog(object())
        )
        ...
        with self.assertRaises(ThirteenthTrancheFixtureCatalogError):
            replace(
                self.catalog,
        ...
        with self.assertRaises(ThirteenthTrancheFixtureCatalogError):
            validate_thirteenth_tranche_fixture_catalog(reordered)
