...
EXPECTED_FOURTH_REGISTRY_SHA256 = (
    "3dc5512139361a275afaf0b57b94528961615f9b4eee22ee6c333cc7d8bf4ea5"
)
EXPECTED_FOURTH_CUMULATIVE_SUITE_SHA256 = (
...
class FourthTrancheTaskRegistryTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.registry = build_fourth_tranche_task_registry()
    ...
    def assert_registry_invalid(self, registry: object) -> None:
        with self.assertRaises(
            (FourthTrancheRegistryError, TypeError, ValueError)
        ):
            validate_fourth_tranche_task_registry(  # type: ignore[arg-type]
                registry
            )

    def test_grid_has_exact_type_order_and_unique_identities(self) -> None:
        tasks = self.registry.added_tasks
        self.assertEqual(len(tasks), FOURTH_TRANCHE_ADDED_TASK_COUNT)
        self.assertEqual(FOURTH_TRANCHE_ADDED_TASK_COUNT, 20)
        self.assertEqual(FOURTH_TRANCHE_CUMULATIVE_TASK_COUNT, 260)
        self.assertEqual(
            FOURTH_TRANCHE_FAMILY_ORDER,
    ...
    def test_rebuild_is_deterministic_and_new_hashes_are_frozen(self) -> None:
        self.assertEqual(
            build_fourth_tranche_added_tasks(), self.registry.added_tasks
        )
        rebuilt = build_fourth_tranche_task_registry()
        ...
        self.assertEqual(
            rebuilt.registry_sha256,
            compute_fourth_tranche_registry_sha256(rebuilt.added_tasks),
        )
        self.assertEqual(
            rebuilt.cumulative_suite_sha256,
            compute_fourth_tranche_cumulative_suite_sha256(
    ...
    def test_all_260_task_identities_are_globally_unique(self) -> None:
        tasks = (
            *self.first.tasks,
        ...
        self.assertEqual(len(tasks), FOURTH_TRANCHE_CUMULATIVE_TASK_COUNT)
