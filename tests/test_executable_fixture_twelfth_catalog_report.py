...
SCRIPT_PATH = (
    ROOT / "scripts" / "build_executable_twelfth_tranche_catalog.py"
)
SCRIPT_SPEC = importlib.util.spec_from_file_location(
    "cbds_build_executable_twelfth_tranche_catalog",
    SCRIPT_PATH,
)
if SCRIPT_SPEC is None or SCRIPT_SPEC.loader is None:
    raise RuntimeError("twelfth-tranche report builder cannot be loaded")
...
REPORT = (
    ROOT / "reports" / "executable-twelfth-tranche" / "manifest.json"
)
EXPECTED_REPORT_SHA256 = (
    "792bb1a4116d6698cc07cebfa6edef9c6358ccd4fe497d99703e88ed81262103"
)
EXPECTED_REPORT_LENGTH = 56_394
...
class TwelfthTrancheCatalogReportTests(unittest.TestCase):
    @classmethod
    ...
    def test_builder_serializes_only_central_bounded_projection(self) -> None:
        with mock.patch.object(
            ...
            first = (
                REPORT_SCRIPT.canonical_twelfth_tranche_catalog_bytes()
            )
            second = (
                REPORT_SCRIPT.canonical_twelfth_tranche_catalog_bytes()
            )
        ...
        self.assertLessEqual(
            len(first), REPORT_SCRIPT.TWELFTH_TRANCHE_REPORT_MAXIMUM_BYTES
        )
        parsed = json.loads(
    ...
    def test_projection_is_hash_only_public_and_nonauthorizing(self) -> None:
        ...
        self.assertEqual(self.record["cumulative_fixture_count"], 2_100)
    ...
    def test_authority_or_size_drift_fails_before_publication(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            oversized = b"x" * (
                REPORT_SCRIPT.TWELFTH_TRANCHE_REPORT_MAXIMUM_BYTES + 1
            )
    ...
    def test_publication_is_idempotent_exact_and_no_replace(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                list(report.parent.glob(".cbds-twelfth-*.tmp")), []
            )
            REPORT_SCRIPT.atomic_publish_noreplace(report, self.payload)
            second = report.lstat()
            self.assertEqual(
    ...
    def test_script_delegates_to_shared_hardened_primitives(self) -> None:
        path = Path("/tmp/cbds-twelfth-report-delegation.json")
        with mock.patch.object(
            REPORT_SCRIPT,
        ...
        publisher.assert_called_once_with(
            path,
            b"{}\n",
            REPORT_SCRIPT.TWELFTH_TRANCHE_TEMPORARY_PREFIX,
        )
        with mock.patch.object(
            REPORT_SCRIPT,
        ...
        reader.assert_called_once_with(
            path,
            REPORT_SCRIPT.TWELFTH_TRANCHE_REPORT_MAXIMUM_BYTES,
        )
