...
class FifthTrancheFullCatalogMaterializationTests(unittest.TestCase):
    def test_all_100_bundles_accept_exact_final_state_and_reject_mutants(self) -> None:
        ...
        with mock.patch.object(
            ...
            validate_fifth_tranche_fixture_catalog(catalog)
            ...
            for index, bundle in enumerate(catalog.bundles):
                ...
                with self.subTest(
                    ...
                    self.assertIs(type(task), PipefailAtomicReportTask)
                    self.assertIs(type(bundle), PipefailAtomicReportFixtureBundle)
                    with materialize_pipefail_atomic_report_fixture(
        ...
        self.assertEqual(family_passes, {PIPEFAIL_ATOMIC_REPORT_FAMILY_ID: 100})
        self.assertEqual(
            profile_passes,
            {
        ...
        self.assertGreater(absent_output_acceptances, 0)
        self.assertGreater(missing_rejections, 0)
        self.assertEqual(
            absent_output_acceptances + missing_rejections,
            100,
        )
        self.assertEqual(unexpected_path_rejections, absent_output_acceptances)
