...
class FifthTrancheFixtureCatalogTests(unittest.TestCase):
    @classmethod
    ...
    def assert_catalog_invalid(self, catalog: object) -> None:
        self.assertFalse(verify_fifth_tranche_fixture_catalog(catalog))
        with self.assertRaises(
            (FifthTrancheFixtureCatalogError, TypeError, ValueError)
        ):
            validate_fifth_tranche_fixture_catalog(  # type: ignore[arg-type]
                catalog
            )
    ...
    def test_exact_types_are_task_major_profile_minor(self) -> None:
        ...
        self.assertEqual(len(bundles), FIFTH_TRANCHE_ADDED_FIXTURE_COUNT)
        self.assertEqual(FIFTH_TRANCHE_ADDED_FIXTURE_COUNT, 100)
        self.assertEqual(FIFTH_TRANCHE_FIXTURE_COUNT, 100)
        self.assertEqual(FIFTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT, 1_400)
        self.assertEqual(FIFTH_TRANCHE_PROFILE_COUNT, 5)
        self.assertTrue(all(type(task) is PipefailAtomicReportTask for task in tasks))
        self.assertTrue(
            all(type(bundle) is PipefailAtomicReportFixtureBundle for bundle in bundles)
        )

        fixture_ids: set[str] = set()
        fixture_hashes: set[str] = set()
        for index, bundle in enumerate(bundles):
            task = tasks[index // FIFTH_TRANCHE_PROFILE_COUNT]
    ...
    def test_rebuild_is_deterministic_hash_bound_and_never_executes(self) -> None:
        with mock.patch.object(
            ...
            rebuilt = build_fifth_tranche_fixture_catalog()
        ...
        self.assertEqual(
            rebuilt.catalog_sha256, EXPECTED_FIFTH_CATALOG_SHA256
        )
        self.assertEqual(
    ...
    def test_all_1400_fixture_identities_are_globally_unique(self) -> None:
        bundles = (
            *self.first.bundles,
            *self.second.bundles,
            *self.third.bundles,
        ...
        self.assertEqual(len(bundles), FIFTH_TRANCHE_CUMULATIVE_FIXTURE_COUNT)
        self.assertEqual(
    ...
    def test_projection_binds_generator_and_is_hash_only(self) -> None:
        ...
        self.assertEqual(record["base_cumulative_fixture_count"], 1_300)
        self.assertEqual(record["added_fixture_count"], 100)
        self.assertEqual(record["cumulative_fixture_count"], 1_400)
        self.assertEqual(record["profiles_per_task"], 5)
        self.assertEqual(
            record["family_task_counts"], {PIPEFAIL_ATOMIC_REPORT_FAMILY_ID: 20}
        )
        self.assertEqual(
            record["family_fixture_counts"], {PIPEFAIL_ATOMIC_REPORT_FAMILY_ID: 100}
        )
        self.assertEqual(
    ...
    def test_order_nested_type_authority_and_spoof_mutations_fail(self) -> None:
        ...
        object.__setattr__(
            wrong_bundle_type,
            "bundles",
            (self.fourth.bundles[0], *self.catalog.bundles[1:]),
        )
